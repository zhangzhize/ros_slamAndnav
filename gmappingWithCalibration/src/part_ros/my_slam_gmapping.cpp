...
/*
 * 1、初始化激光雷达坐标系和odom坐标系的tf
 * 2、初始化获取launch参数的句柄
 * 3、初始化scan_filter、scan_filter_sub_话题
 * 4、初始化odom和map的tf发布线程
 */
...
/*  1、定义map、odom、laser坐标系的名称
 *  2、创建运动畸变去除对象
 *  3、创建gmapping对象
 *  4、创建tf的broadcaster，发布odom和map的tf
 *  5****、加载gmapping相关参数，包括激光雷达
 */
...
MySlamGMapping::~MySlamGMapping()
{
    ...
    delete gsp_;         //gsp=GridSlamProcessor
    delete lmc_;         //lmc=LidarMotionCalibrator
 ...
 * 		2、元地图
 * 初始化提供的动态地图服务
 * 订阅激光雷达数据，注册回调函数laserCallback(),并进行坐标转换(target_frame是odom_frame)
 * 	获取到的激光数据后首先通过消息过滤器，实现激光扫描数据的订阅以及tf、frame变换，之后激光数据消息处理回调函数
 * 发布map到odom的转换关系的线程
*/
void MySlamGMapping::startLiveSlam()
{
    map_publisher_ = node_.advertise<nav_msgs::OccupancyGrid>("map", 1, true);
    mapmeta_publisher_ = node_.advertise<nav_msgs::MapMetaData>("map_metadata", 1, true);
...
//    ROS_DEBUG("Start transform_thread ");
    std::cout<<"Start transform_thread（开启tf发布线程！！） "<<std::endl;
}

//每当到达一帧scan数据，就将调用laserCallback函数
/*
 * 1、运动畸变处理后，判断激光数据是否为第一帧，当为第一帧时初始化地图，并修改标志位
 * 						调用intMapper()
 * 2、调用addScan()函数，添加激光数据，若返回值为真执行3
 * 3、获取2中处理的数据后，得到粒子最佳匹配位置
 * 4、将3中获取的位姿用于激光到地图，里程即到地图，地图到里程即的tf变换
 * 5、执行地图更新
 */
void MySlamGMapping::laserCallback(const sensor_msgs::LaserScan::ConstPtr& scan)
{
    ...
    //std::cout<<"相邻激光束时间间隔是:"<<laserScanMsg.time_increment<<std::endl;
    //std::cout<<"相邻激光束角度间隔是:"<<laserScanMsg.angle_increment<<std::endl;
    laser_ranges_.clear();
    laser_angles_.clear();
    //拷贝scan数据到laser_ranges_,laser_angles_
    double lidar_dist,lidar_angle;
    //存储一圈激光数据到laser_ranges_和laser_angles_的vector中
...
/*输入：scan   返回：pose、processScan()函数的返回值
 * 1、确保获取到上一次的里程计的位姿、时间戳以及相同数量的激光扫描点
 * **2、判断是否需要将激光扫描逆向反转,此处省略
 * 2、执行RangeReading(),读取点云大小、点云扫描值、gsp_laser_(RangeSensor()返回值)、单位为秒的时间戳到reading，并设置reading的位姿
 * 3、执行processScan(reading)即slam处理程序
 */
bool MySlamGMapping::addScan(const sensor_msgs::LaserScan& scan, GMapping::OrientedPoint& gmap_pose)
{
    ...
    //把ROS的激光雷达数据信息 转换为 GMapping算法看得懂的形式,这里是加入运动畸变去除之后的激光点的角度数据
...
void MySlamGMapping::updateMap(const sensor_msgs::LaserScan& scan)
{
    ...
    mapmeta_publisher_.publish(map_.map.info);
}
