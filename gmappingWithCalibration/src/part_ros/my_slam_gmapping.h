...
{
  public:
    MySlamGMapping();     //构造函数，创建对象时调用
    void init();          //构造函数调用的初始化函数
    ~MySlamGMapping();    //析构函数，删除对象时调用
    
    void startLiveSlam(); //启动实时SLAM 
    ...
    void publishLoop(double transform_publish_period);                                   //延时函数控制发布周期
    void publishTransform();                                                             //发布map和odom的TF变换
  ...
  private:
    ...
    ros::Publisher map_publisher_;             //发布话题map
    ros::Publisher mapmeta_publisher_;         //发布话题map_metadata
    ros::ServiceServer ss_;                    //发布服务dynamic_map
    //以下三行组合使用
    tf::TransformListener tf_;                 //tf变换的监听者，监听？？？                 
